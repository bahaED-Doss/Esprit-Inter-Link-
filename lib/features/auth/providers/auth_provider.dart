...
// 🚀 NOUVELLES MÉTHODES ADMIN
  ...
  // --- Connexion ---
  Future<bool> login({required String email, required String password, BuildContext? context}) async {
    _isLoading = true;
    _error = null;
    ...
    try {
      ...
      if (user != null) {
        ...
        // Persist user session using UserSessionProvider if context provided
        if (context != null) {
          try {
            final sessionProvider = Provider.of<UserSessionProvider>(context, listen: false);
            // user.id might be null for some flows; convert to string
            final userIdStr = user.id?.toString() ?? '';
            final roleStr = user.role.toLowerCase();
            // Map role string to UserRole enum
            final userRole = userRoleFromString(roleStr);
            ...
            // ignore session persistence errors but log
